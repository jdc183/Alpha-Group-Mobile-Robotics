...
void set_example_object_frames(geometry_msgs::PoseStamped &object_poseStamped,
    ...
    object_poseStamped.pose.position.x = 0.548534;
    object_poseStamped.pose.position.y = -0.1202087;
    object_poseStamped.pose.position.z = 0.78863; //-0.125; //pose w/rt world frame
    object_poseStamped.pose.orientation.x = 0;
    object_poseStamped.pose.orientation.y = 0;
    object_poseStamped.pose.orientation.z = 1;
    object_poseStamped.pose.orientation.w = 0;
...
bool serviceCB(alpha_final::ObjectGrabberServiceRequest& request, alpha_final::ObjectGrabberServiceResponse& response){
    geometry_msgs::PoseStamped object_pickup_poseStamped;
    geometry_msgs::PoseStamped object_dropoff_poseStamped;
    
    object_pickup_poseStamped.pose = request.des_pose;
    object_dropoff_poseStamped.pose = request.des_pose;
    
    //will require that "system_ref_frame" is known to tf
    object_pickup_poseStamped.header.frame_id = "system_ref_frame"; //set object pose; ref frame must be connected via tf
    object_dropoff_poseStamped.header.frame_id = "system_ref_frame"; //set object pose; ref frame must be connected via tf
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "example_object_dropper_action_client");
    ros::NodeHandle nh;
    ...
    ros::ServiceServer service = nh.advertiseService("dropppation_service",serviceCB);
