...
double angle_min_= 5*PI/6;//Rightmost angle of interest
double angle_max_= 7*PI/6;//Leftmost angle of interest
double angle_increment_=0.0;
double range_min_ = 0.06;//minimum distance of laser_0
double range_max_ = 1.0;//maximum distance of interest
    ...
    for (int ping_index_ = min_ping_index; ping_index_ <= max_ping_index; ping_index_++){
    	ping_dist_in_front_ = laser_scan.ranges[ping_index_];
   		...
   		if (ping_dist_in_front_<range_max_alarm && ping_dist_in_front_ > range_min_alarm) {
       		no_wall_on_left = true; // dont want an alarm, just want to set something to trigger if we need to turn
   		}	
    }

    //sends warning if data point trips lidar alarm
    if (no_wall_on_left = true){
